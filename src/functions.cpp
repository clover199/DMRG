                            ...
                            vector<string>& para_name)
{
  ...
  for(int i=0;i<argc_;i++)
  {
    ...
    for(int j=0;j<para_name.size();j++)// if(str_bef.compare(para_name) == 0)
      ret[j] = atof(str_aft.c_str());
  }
  
  cout << "\n ***** The parameters for this running ***** \n"
          "\t sites(L) = " << sites << "\t cutoff(m) = " << cutoff << "\t sweep(n) = " << sweep << endl;
  for(int i=0;i<ret.size();i++) cout << "\t" << para_name[i] << " = " << ret[i] << endl << endl;
  
  std::stringstream ss;
  ss << "_L_" << sites << "_m_" << cutoff << "_n_" << sweep;
  for(int i=0;i<ret.size();i++) cout << "_" << para_name[i] << "_" << ret[i];
  filename = ss.str();
